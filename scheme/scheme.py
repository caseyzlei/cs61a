...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        args = expr.rest.map(lambda func: scheme_eval(func, env))
        return scheme_apply(first_expr, args, env)
        # END PROBLEM 4
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        lam = LambdaProcedure(target.rest, expressions.rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        ...
        evald = scheme_eval(expressions.first, env)
        if is_false_primitive(evald):
            return False
        else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        one_val = scheme_eval(bindings.first.rest.first, env)
        values = Pair(one_val, values)
