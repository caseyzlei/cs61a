...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        new_place = self.place # REPLACE THIS LINE
        new_bees = new_place.bees
        transition = 0

        while new_place is not beehive:
            if len(new_bees) != 0 and self.min_range <= transition <= self.max_range:
                return rANTdom_else_none(new_bees)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting > 0:
            self.digesting -= 1
        elif not self.digesting:
            if len(my_place.bees) !=0:
                self.eat_bee(rANTdom_else_none(my_place.bees))
            else:
                pass
        # END Problem 6
...
# END Problem EC
    ...
    def __init__(self, armor=1):
        ...
        if not QueenAnt.has_been_called:
            self.impostor = False
        else:
            self.impostor = True
        QueenAnt.has_been_called = True
    ...
    def action(self, gamestate):
        ...
        if self.impostor:
            ...
            while curr != None:
                if curr.ant != None and not curr.ant in QueenAnt.doubled_ants:
                    QueenAnt.doubled_ants.append(curr.ant)
                    curr.ant.damage = curr.ant.damage * 2
                curr = curr.exit
        # END Problem EC
